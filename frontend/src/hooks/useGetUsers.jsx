...
const useGetUsers = () => {
    ...
    useEffect(()=>
    {
        const getUsers=async()=>
        {
            setloading(true);
            try {
                const res=await fetch("http://localhost:8000/api/auth/getusers",{
                    method:"GET",
...
// Side Effects: The useEffect hook is designed to handle side effects, such as data fetching, subscriptions, or manually changing the DOM.
...
// Avoiding Infinite Loops: If you call the getUsers function directly inside the component body, it would run on every render, potentially causing an infinite loop.
